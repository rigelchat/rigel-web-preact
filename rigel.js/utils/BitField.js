import { RigeljsRangeError, ErrorCodes } from "../errors.js";
...
class BitField {
    ...
    static resolve(bit) {
        ...
        if (Array.isArray(bit)) {
            return bit.map((b) => this.resolve(b)).reduce((prev, b) => prev | b, DefaultBit);
        };
        if (typeof bit === "string") {
            if (!isNaN(bit)) return typeof DefaultBit === "bigint" ? BigInt(bit) : Number(bit);
            if (this.Flags[bit] !== undefined) return this.Flags[bit];
        };
        throw new RigeljsRangeError(ErrorCodes.BitFieldInvalid, bit);
    };
