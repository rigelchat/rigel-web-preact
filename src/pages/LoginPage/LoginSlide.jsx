...
import { openDiscordLoginPopup } from "../../utils/index.js";

export default function LoginSlide({ instanceInfo, serverConfig, envConfigExists, showPreviousSlide }) {
    const { goToGuildDiscovery } = useContext(RouteContext);
    ...
    const handleSubmit = useCallback((evt) => {
        ...
        axios(`${serverConfig.api.baseUrl}/api/auth/login`, {
            method: "POST",
            data: { login, password }
        }).then((res) => {
            storage.safeClear();
            storage.setInstanceConfig({
                api: serverConfig.api.baseUrl + "/api",
                cdn: serverConfig.cdn.baseUrl,
                version: serverConfig.api.apiVersions?.default,
                compress: serverConfig.gateway?.compression?.[0]
            });
    ...
    const handleDiscordLogin = useCallback((evt) => {
        evt.preventDefault();

        const { clientId, authUrl } = instanceInfo.authMethods.discord;
        ...
        <div className={classNames(modalStyles.modal, styles.authBox)}>
            ...
            {instanceInfo.name && (
                <div className={modalStyles.footer}>
                    {!envConfigExists && <Button variant="link" size="mini" onClick={showPreviousSlide}>Retour</Button>}
                    <div className={styles.instanceInfo}>
                        {instanceInfo.image && <img src={instanceInfo.image} className={styles.image} />}
                        {instanceInfo.name}
                    </div>
                </div>
            )}
